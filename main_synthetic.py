...
import util.misc as utilsf
import wandb
from datasets import build_dataset, get_coco_api_from_dataset
from engine import evaluate, evaluate_CTC, test, train_one_epoch, train_one_epoch_CTC
...
def main(args):
    ...
    if args.new_class_embedding and not args.resume_finetuning:
        ...
        new_label_enc = nn.Embedding(len(dataset_val.charset) + 1, features_dim).to(
            device
        )

        if args.smart_mapping:
            old_charset = pickle.load(
                open("/home/rbaena/projects/OCR/DINO/datasets/charset_symbol.pkl", "rb")
            )
            ...
            for j in range(model.transformer.num_decoder_layers):
                for i in range(new_charset_size):
                    new_decoder_class_embed[j].weight.data[
                        i, :
                    ] = model.transformer.decoder.class_embed[j].weight.data[
                        mapping[i], :
                    ]
                    new_decoder_class_embed[j].bias.data[
                        i
                    ] = model.transformer.decoder.class_embed[j].bias.data[mapping[i]]

                    new_enc_out_class_embed.weight.data[
                        i, :
                    ] = model.transformer.enc_out_class_embed.weight.data[mapping[i], :]
                    new_enc_out_class_embed.bias.data[
                        i
                    ...
                    new_label_enc.weight.data[i, :] = model.label_enc.weight.data[
                        mapping[i], :
                    ]

        model.transformer.decoder.class_embed = new_decoder_class_embed.to(device)
        model.class_embed = new_class_embed.to(
            device
